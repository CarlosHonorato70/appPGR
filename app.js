document.addEventListener('DOMContentLoaded', () => {
    ...
    const ENCRYPTION_KEY = 'PGR_SECURITY_KEY_2025'; // Chave para criptografia AES
    
    // Credenciais de demonstração (em produção, use backend seguro)
    ...
    const encryptData = (data) => {
        try {
            if (typeof CryptoJS !== 'undefined') {
                ...
                console.warn('CryptoJS não disponível. Usando armazenamento sem criptografia.');
                return JSON.stringify(data);
            }
        } catch (error) {
            console.error('Erro ao criptografar dados:', error);
            return JSON.stringify(data); // Fallback para JSON simples
        }
    };
    ...
    const decryptData = (encryptedData) => {
        try {
            ...
            // Tentativa de fallback para dados em JSON simples
            try {
                return JSON.parse(encryptedData);
            } catch {
                return [];
    ...
    // --- Funções Auxiliares Modificadas com Criptografia ---
    const getStoredData = (key) => {
        const encryptedData = localStorage.getItem(key);
        if (!encryptedData) return [];
        
        // Verifica se o dado já está criptografado (compatibilidade)
        try {
            // Tenta descriptografar primeiro
            return decryptData(encryptedData);
        } catch {
            try {
                // Se falhar, tenta como dados não criptografados (compatibilidade)
                return JSON.parse(encryptedData);
            } catch {
                return [];
            }
    ...
    // --- Função para inicializar a aplicação (apenas se autenticado) ---
    const initializeApp = () => {
    ...
    const checkAuthStatus = () => {
        const sessionUser = sessionStorage.getItem('pgr_authenticated_user');
        if (sessionUser) {
    ...
    const showMainApp = () => {
        ...
        document.getElementById('logged-user').textContent = `Usuário: ${currentUser}`;
        initializeApp();
    };
    ...
    const processLogin = (username, password) => {
        if (DEMO_CREDENTIALS[username] && DEMO_CREDENTIALS[username] === password) {
            ...
            sessionStorage.setItem('pgr_authenticated_user', username);
            showMainApp();
            return true;
        }
        return false;
    ...
    const processLogout = () => {
        currentUser = null;
        sessionStorage.removeItem('pgr_authenticated_user');
        showLoginScreen();
    };
    ...
    document.getElementById('login-form').addEventListener('submit', (e) => {
        ...
        if (processLogin(username, password)) {
            ...
            errorDiv.textContent = 'Usuário ou senha inválidos. Verifique as credenciais de demonstração abaixo.';
